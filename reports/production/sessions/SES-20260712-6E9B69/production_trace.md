# Production Trace

**Generated:** 2026-07-12T03:53:06+00:00
**Mission:** corporate governance — service knowledge for Corporate Governance — continuous knowledge manufacturing for service_library across enterprise function Corporate Governance (function_gap=58.4; not BD-only); dataset_gap=112.922; mode=BOOTSTRAP
**Mission ID:** `MIS-20260712-312979`
**Session ID:** `SES-20260712-6E9B69`
**Started:** 2026-07-12T03:35:01+00:00
**Finished:** 2026-07-12T03:53:06+00:00

## Pipeline timeline

| Stage | Status | Duration (ms) | Docs | Rows | Errors |
|-------|--------|--------------:|-----:|-----:|--------|
| mission | completed | 1.4 | 0 | 0 | — |
| source_discovery | completed | 2.9 | 0 | 0 | — |
| connector | completed | 94147.6 | 235 | 0 | — |
| document_discovery | completed | 94147.8 | 235 | 0 | — |
| document_download | completed | 213300.7 | 147 | 0 | — |
| extraction | completed | 97.5 | 0 | 5 | — |
| candidate_validation | completed | 7.3 | 0 | 5 | — |
| publish_queue | completed | 7.3 | 0 | 5 | — |
| append_dataset | completed | 79.8 | 0 | 5 | — |
| export | skipped | 0.4 | 0 | 0 | — |
| git_commit | skipped | 0.4 | 0 | 0 | — |
| push | skipped | 0.3 | 0 | 0 | — |

## Summary

- Documents discovered: **31**
- Documents downloaded: **147**
- Candidates extracted: **5**
- Candidates validated: **5**
- Candidates rejected: **0**
- Rows published: **5**
- Duplicates: **0**
