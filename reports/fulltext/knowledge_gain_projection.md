# Knowledge Gain Projection

**Generated:** 2026-07-14T18:26:17+00:00

## Before (Forensic Audit 002)

| Metric | Value |
|--------|------:|
| Metadata-only | 98.1% |
| Rows / document | 0.93 |
| Potential rows / document | 6.48 |
| DOI → full text | 0.0% |
| Mean usable words | 97 |

## After (this session / simulation)

| Metric | Value |
|--------|------:|
| Metadata % | 50.63 |
| Full-text % | 3.8 |
| DOI full-text rate | 47.37 |
| Avg content size (chars) | 4227.7 |
| Avg richness | 58.57 |

## Projected extraction yield (no extractor changes)

| Model | Projected rows/doc |
|-------|-------------------:|
| Content-size uplift proxy | ~3.41 |
| Full-text mix proxy (70% of potential on FT docs) | ~1.07 |
| Target success criteria | ≥5.0 |

Actual rows still depend on mission-scoped extraction; richer bodies unlock higher potential for a later multi-dataset extract sprint.
