...
def build_dataset_card(
    *,
    ...
    daily = _read_json(repo_root / f"automation/learning/state/daily_{datetime.now(timezone.utc).strftime('%Y-%m-%d')}.json")
    ...
    ds_lines="\n".join(
        f"| `{d['name']}` | {d['rows']} | {d['source']} |"
        for d in datasets
    ) or "| - | 0 | - |"

    mode=manufacturing.get("mode") or {}
    mode_s=mode.get("mode") if isinstance(mode,dict) else str(mode)

    citation=f"""@dataset{{ida_dataset_{version.replace(".", "_")}}},
